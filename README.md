**これは UMA の時分割タイミングを変更したバージョンです。PLLのクロックソースを27MHzにし、UMAのタイミングをMSXの3.58MHzと同期させています。T80を使用したマシンでNextorがハングアップする現象が改善される可能性がありますが、V9990 の映像は正常に出力されないでしょう(27Mhzと3.58MHzの誤差が少ない場合は正常な映像が出力されますが...)。**


また、V9990 のテストコードが含まれるので、白い縦線が表示されます。
